...
If your prompt starts with `-`, insert `--` before it so CLI parsing stops at that point:
...
- **Wrapper install:** `~/.local/bin/crsr` from `npm run release` (Unix-like systems) is detected and refused, because it should be rebuilt from source instead of overwritten with a downloaded binary.
