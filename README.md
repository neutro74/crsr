...
Source-checkout wrappers installed by `npm run release` are intentionally **not** self-update targets. They point back to the repository bundle, so `crsr --update` now refuses to overwrite that wrapper and tells you to rebuild from source instead.
