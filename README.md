...
Screenshot assets are not bundled in this source checkout. The release README on GitHub includes the current UI captures.
...
npm run bundle
mkdir -p release
npx pkg package.json --targets node18-linux-x64 --output release/crsr-linux-x64 --public-packages '*'
npx pkg package.json --targets node18-macos-x64 --output release/crsr-macos-x64 --public-packages '*'
npx pkg package.json --targets node18-macos-arm64 --output release/crsr-macos-arm64 --public-packages '*'
npx pkg package.json --targets node18-win-x64 --output release/crsr-win-x64.exe --public-packages '*'
```
...
- **Wrapper installs from `npm run release`:** these stay source-based and are not overwritten by self-update; rerun `npm run release` after rebuilding instead.
