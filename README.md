...
**Linux, macOS (Intel + Apple Silicon), and Windows x64 in one step** (same targets as the release pipeline):
...
- **Wrapper install:** `~/.local/bin/crsr` from `npm run release` (Unix-like systems) is detected and refused, because replacing the shell wrapper with a packaged binary would break source installs.
