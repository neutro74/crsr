...
The updater downloads the **latest** GitHub release and replaces the active `crsr` executable when it can resolve an updatable install path:
...
If `crsr` was installed from source via `npm run release`, self-update now refuses to replace that local wrapper with a packaged release binary. Rebuild from source instead:
