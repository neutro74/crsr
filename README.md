...
**Linux, macOS x64/arm64, and Windows x64 in one step** (same targets as the release pipeline):
...
If you installed the local Node wrapper via `npm run release`, self-update will refuse to overwrite that wrapper script. Rebuild from source with `npm run release`, or point `CRSR_INSTALL_PATH` at a standalone release binary before using `crsr --update`.
