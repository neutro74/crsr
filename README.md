...
- **Wrapper install:** `~/.local/bin/crsr` from `npm run release` (Unix-like systems). This is a shell wrapper, so `--update` refuses to overwrite it; rebuild from source with `npm run release`, or point `CRSR_INSTALL_PATH` at a standalone binary.
