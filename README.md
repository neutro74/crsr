...
- `--`: stop parsing options and treat the rest as the prompt verbatim
...
- **`CRSR_INSTALL_PATH`:** explicit path to the packaged binary to replace.
- **Source wrapper install:** `~/.local/bin/crsr` from `npm run release` is detected and left alone.

If you installed from source with `npm run release`, rebuild the wrapper from this checkout instead of self-updating it:

```bash
npm run release
```

If you want `--update` to manage a standalone binary, point `CRSR_INSTALL_PATH` at that packaged executable path before running the update.
